...
float** metodos::reset_Matrix(int rows, int cols,float val1){
    int row,col;
    float **IDW = new float *[rows];//matriz IDW para todas las localidades
    for (row = 0; row < rows; row++)
        IDW[row] = new float[cols];
    for(row=0; row<rows; row++)
...
void metodos::IDW_test(float req, float **cost_dist, float **&IDW, int rows, int cols, float exp, int cell_null){
    ...
    for(row = 0; row < rows; row++)
        for(col = 0; col < cols; col++){
            if(cost_dist[row][col]<=0)
                IDW[row][col]=cell_null;
            else
                IDW[row][col] += req / pow(cost_dist[row][col], exp);
        }
}
//------------------------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------
float** metodos::cost_distance(float **fric_matrix, int rows, int cols, position array, float **CD_matrix) {
    ...
    for(row_temp=0;row_temp<rows;row_temp++)
        for(col_temp=0;col_temp<cols;col_temp++)
            CD_matrix[row_temp][col_temp]=std::numeric_limits<float>::max();
    position inicial;
    ...
    while(!CD_costos.empty()){
        ...
        for(i=1;i<9;i++){
            ...
            if (row_temp < rows && row_temp >= 0 && col_temp < cols && col_temp >= 0 && fric_matrix[row_temp][col_temp]>0.0) {
                if (i % 2 != 0)//si es un movimiento lateral
                    array.val_fricc = inicial.val_fricc + (fric_matrix[row_temp][col_temp]);
                else//si es un movimiento diagonal
                    array.val_fricc =  inicial.val_fricc + sqrt(2) * (fric_matrix[row_temp][col_temp]);
                //se busca el menor valor de CD, es posible que se escriba varias veces en una celda
                if (CD_matrix[row_temp][col_temp]>array.val_fricc) {
                    array.row = row_temp;
                    array.col = col_temp;
                    array.key=key;
                    key++;
                    CD_matrix[array.row][array.col] = array.val_fricc;
                    CD_costos.push(array);
                }
            }
        }
